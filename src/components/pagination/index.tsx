  ...
  function renderProps () {
    return pagination.map(pag => {
      ...
      if(pag.id === props.selected) {
        return (
          <li className={styles.selectedButton} key={pag.id}>
            <Link aria-label={`Navega para a página ${pag.id}`} to={generateUrl(pag.id)} className={styles.active}>{pag.id}</Link>
          </li>
        )
      ...
      // ou se a posição em análise for <= a posição selecionada + 2
        ...
        return (
          <li className={styles.selectedButton} key={pag.id}>
            <Link aria-label={`Navega para a página ${pag.id}`} to={generateUrl(pag.id)}>{pag.id}</Link >
          </li>
        )
